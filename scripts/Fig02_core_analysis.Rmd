...
#geom_bar(color="black",stat="identity", width=0.8) + #from Morten
  labs( x = "WWTPs",
        y = "Cumulative read abundance \nat ASV level [%]",
        fill = "Core group",
        tag = 'A') +
...
geom_bar(stat = "identity") +
  geom_text(data = summary_asv_avg, aes(label = paste0(round(mean_cum_rra_asv, digits = 1), ' % (', Number_ASVs,')')),
...
geom_bar(stat = "identity") +
  geom_text(data = summary_sp_avg, aes(label = case_when(Species_type == 'Unclassified' ~ paste0(round(mean_cum_rra_sp, digits = 1), ' %'),
                                                       TRUE ~ paste0(round(mean_cum_rra_sp, digits = 1), ' % (', Number_spp,')'))),
             size = 3, position = position_stack(vjust = 0.5)) +
#geom_bar(color="black",stat="identity", width=0.8) + #from Morten
...
geom_bar(stat = "identity") +
  geom_text(data = summary_gen_avg, aes(label = case_when(Genus_type == 'Unclassified' ~ paste0(round(mean_cum_rra_gen, digits = 1), ' %'),
                                                       TRUE ~ paste0(round(mean_cum_rra_gen, digits = 1), ' % (', Number_gen,')'))),
             size = 3, position = position_stack(vjust = 0.5)) +
#geom_bar(color="black",stat="identity", width=0.8) + #from Morten
  labs( fill = 'Core group') +
    scale_fill_manual(#palette="RdYlBu", na.value = "grey"
