---
title: 'Figure S5+6: Ordination for core + CRAT'
...
##subset
```{r}
# change according to which group is to be investigated
core_CRAT_ASVs <- corecrat_ASV$OTU # Core + CRAT
core_ASVs <- corecrat_ASV %>% filter(!Category %in% 'CRAT') %>% pull(OTU) # Only core
CRAT_ASVs <- corecrat_ASV %>% filter(Category %in% 'CRAT') %>% pull(OTU) # Only CRAT


dasn_core_crat <- amp_subset_taxa(dasn,
                                     tax_vector = core_ASVs) # change here what you want to subset to
...
ggsave(filename="output/FigS5_PCoA_core_AS_bray_parameters.png", 
       p,
       width=7.8, height=2.6,
       dpi = 320)

ggsave(filename="output/FigS5_PCoA_core_AS_bray_parameters.pdf", 
       p,
