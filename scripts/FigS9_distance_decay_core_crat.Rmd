...
###prepare data
```{r}
ASabund <- t(dasn_CRAT$abund) #transpose the abundance dataframe #change df

#longitude and latitute table
AS_geo <- dasn_CRAT$metadata %>%  #change df
...
p <- plot_grid(p_AS_bray + labs(tag= 'A')+ theme(plot.tag = element_text(face='bold')), 
                      ...
                      nrow = 2,
                      rel_widths = c(1, 1))
ggsave(filename = "output/FigSxx_DDR_ASV_CRAT.jpeg", height = 7, width = 7,limitsize=FALSE)
ggsave(filename = "output/FigSxx_DDR_ASV_CRAT.pdf", height = 7, width = 7, useDingbats=FALSE, limitsize=FALSE)
```
